    ...
    const selectBestCrop = (crops: MasterCrop[], slotKey: string): MasterCrop | null => {
      ...
      const scored = crops
        .filter(c => {
          ...
          // RULE 3 — Pathological: same-genus heavy feeders blocked to prevent
          // nightshade clustering (e.g. Tomato + Pepper + Eggplant in same bed).
          // Requires BOTH crops to have spacing > 36" for an exception.
          ...
          if (cGenus && c.category === 'Sustenance') {
            ...
            for (const placed of placedCrops) {
              if (placed.category === 'Sustenance' && getGenus(placed) === cGenus) {
                ...
                if (cSpacing <= 36 || placedSpacing <= 36) return false;
              }
            }
          }
