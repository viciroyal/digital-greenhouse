...
import { MineralEarthIcon } from './GemstoneIcons';
...
const EarthQuadrant = ({ track }: EarthQuadrantProps) => {
  ...
  return (
    <motion.div
      className="forest-card p-6 h-full"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.3 }}
    >
      <div className="flex items-center gap-3 mb-4">
        <MineralEarthIcon color={track.colorHsl} size={28} />
        <h3 className="font-bubble text-xl text-foreground">The Earth</h3>
        <span className="text-muted-foreground/60 font-body text-xs ml-auto tracking-wider">MINERAL</span>
      </div>
      ...
      <div className="flex flex-col items-center justify-center py-4">
        ...
        >
          ...
          <motion.div
            className="relative w-32 h-32 md:w-40 md:h-40 overflow-hidden rounded-2xl transition-all duration-300"
            style={{
              boxShadow: isHovered 
                ? `0 0 30px hsl(${track.colorHsl} / 0.7), 0 0 60px hsl(${track.colorHsl} / 0.5), 0 8px 32px hsl(${track.colorHsl} / 0.4)`
                : `0 8px 32px hsl(${track.colorHsl} / 0.3), 0 0 0 2px hsl(${track.colorHsl} / 0.2)`,
            }}
            animate={{
        ...
        >
          <p className="font-body text-lg md:text-xl font-bold text-foreground tracking-wide">
            {track.mineral}
          </p>
          <p 
